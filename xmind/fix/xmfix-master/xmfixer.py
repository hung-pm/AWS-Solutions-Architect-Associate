...
class DirSource(object):
    ...
    def read(self, entry_name):
        if entry_name.endswith("/"):
            return ""
        # print(os.path.join(self.file_path, entry_name))
        with open(os.path.join(self.file_path, entry_name), "rb") as f:
            return f.read()
...
class XMindFileFixer(object):
    ...
    def rebuild_content(self):
        ...
        json_content_file = os.path.join(self.unzipped_dir, "content.json")

        with open(json_content_file, 'r', encoding='utf-8') as file:
            data = json.load(file)

        def traverse_topic(topic):
            # Duyệt qua các topic con để lấy thông tin
            if "title" in topic.keys():
                topic_data = {"title": topic['title'], "topics": []}
                topic['title'] = f"[{topic['title']}]"
            else:
                return
            if "children" in topic.keys() and "attached" in topic["children"].keys():
                for subtopic in topic["children"]["attached"] or []:
                    topic_data["topics"].append(traverse_topic(subtopic))
            return topic_data

        print(traverse_topic(data[0]['rootTopic']))
        print(data)

        # Ghi dữ liệu vào tệp JSON
        with open(json_content_file, 'w', encoding='utf-8') as file:
                    ...
                    for rev_file_name in os.listdir(revision_dir):
                        ...
                        if m:
                            ...
                            if rev > revision:
                                ...
                                try:
                                    with open(revision_file, "r", encoding="utf-8") as rf:
